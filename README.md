...
MySQL-Operator is a kubernetes operator with the goal to create and manage a MySQL cluster within Kubernetes. This operator will set up a StatefulSet consisting of 3 database nodes which will be group replicated using MySQL Group Replication to achieve High Availability (HA). 
...
To manage fields of the Custom Resource (CR) and Custom Resource Definition(CR) the file [mysql_types.go](/api/v1alpha1/mysql_types.go) This file contains the MySQL-operator API.

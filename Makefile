...
.PHONY: docker-build
docker-build: ## Build docker images for the manager and init container.
...
docker-build-init: ## Build docker image for the init container.
	$(CONTAINER_TOOL) build -t ${INIT_IMG} -f images/init/Dockerfile .
...
# Helmify is a alternative to using the Operator Lifecycle manager; It will generate the corrisponding Helm charts needed to deploy
# the operator to a kubernetes cluster. Using 'make helmify' you can install helmify locally. 
# using 'make helm' you can generate the helm charts
# This option is provided from the following git repository: https://github.com/arttor/helmify
...
CHART_VERSION ?= $(shell echo $(GIT_TAG) | sed -e 's/^v//')
...
helm: manifests kustomize helmify
	$(KUSTOMIZE) build  $(KUSTOMIZE_DIR) | $(HELMIFY) $(CHART_NAME)
